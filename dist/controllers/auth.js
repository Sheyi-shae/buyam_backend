...
export const googleCallback = async (req, res, next) => {
    try {
        const user = req.user;
        if (!user)
            return res.redirect(`${process.env.FRONTEND_URL}/signin&signup-auth/failure`);
        const redirectTo = typeof req.query.state === "string" &&
...
export async function refreshTokenHandler(req, res, next) {
    ...
    try {
        ...
        if (session.expiresAt < new Date()) {
            ...
            const error = new Error('Refresh  required');
            error.statusCode = 401;
            throw error;
        }
