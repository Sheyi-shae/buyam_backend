...
export const createProduct = async (req, res, next) => {
    const { name, price, avatar, condition, negotiable, state, sellerPublicId, city, description, sellerId, categoryId, subCategoryId } = req.body;
    try {
        //slugify name
        ...
        if (existingProduct) {
            const error = new Error('Item already exists in you catalog');
...
export const deleteProductBySlug = async (req, res, next) => {
    const { slug } = req.params;
    const userId = req.user?.id;
    try {
        const product = await db.product.findUnique({
...
export const updateProductBySlug = async (req, res, next) => {
    const { slug } = req.params;
    try {
        const { name, price, avatar, tags, description, sellerId, categoryId, subCategoryId } = req.body;
        //slugify name
...
export const searchProducts = async (req, res, next) => {
    try {
        const { search = "", state, city, page = "1", limit = "10", suggestions, } = req.query;
...
export const getProductsBySubCategorySlug = async (req, res, next) => {
    try {
        const { slug } = req.params;
        const { search = "", suggestions, manual, page = "1", limit = "10", state, city, } = req.query;
        const pageNumber = parseInt(page) || 1;
        const pageSize = parseInt(limit) || 10;
        const skip = (pageNumber - 1) * pageSize;
...
export const updateProductStatusBySlug = async (req, res, next) => {
    const { slug } = req.params;
    const userId = req.user?.id;
